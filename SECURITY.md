...
| The DEK, second copy (only where Face ID or Touch ID is turned on) | The `passkey` setting on that one device; never in a file or the folder | Wrapped with an AES-GCM key derived by HKDF-SHA-256 from a passkey's PRF output, which the device's own authenticator gives only after user verification (`lib/passkey.ts`) |
...
| Someone who can pass the device's own lock (Face ID, Touch ID or its code), where Face ID unlock is on | Open the app, like the user | Open a copy or the folder, or the app on another device: the passkey copy stays on this one |
...
| Face ID or Touch ID in place of the passphrase, per device and off by default (elzacka, 24 September 2026) | A phone closes the app on its own, and a passphrase typed at every start is a cost. The passkey is made on the device's platform authenticator with user verification required; its PRF secret wraps a second copy of the DEK, kept on that device only. No challenge is checked, since there is no server: the protection is that the secret is released only after verification. A copy wrapping another DEK (after a restore or a folder took over another vault) opens nothing and is deleted. Turning it off deletes the copy; the passkey itself stays in the device's password list until the user removes it |
...
| A07 | Authentication Failures                | The passphrase, and on a device where it is turned on, a passkey | Passkey: platform authenticator only, user verification required, the key accepted only if its key id matches the vault's. Passphrase: Never stored or logged; Argon2id bounds offline guessing; no lockout counter because guessing happens offline against the file anyway; idle lock covers the abandoned-device case. Inherent limit: JavaScript strings cannot be zeroed, so the passphrase lingers in memory until garbage collection                                                                                                                                                                                                  |
...
| A10 | Mishandling of Exceptional Conditions  | Corrupt files, hostile parameters, render errors, a slow or hostile catalogue | A lookup has an 8 s timeout, `redirect: 'error'` (the one Facts call follows redirects, since the four Facts hosts redirect between themselves, and the CSP still limits where it may land), `credentials: 'omit'`, `referrerPolicy: 'no-referrer'` and `cache: 'no-store'`, and every failure becomes one of four messages; KDF parameters read from a file are bounded (`m` up to 256 MiB, `t` up to 10, `p` up to 4) before any derivation runs; one undecryptable row does not block the rest; an error boundary shows a message and a reload button instead of a blank page; wrong passphrase, bad JSON and schema failures are caught and shown                                                                                                                                                                              |
