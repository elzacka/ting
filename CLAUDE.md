...
| `src/lib/summary.ts` | The line above the table: totals per kr property over the rows on screen, so a filtered register says what the filter leaves (elzacka, 22 September 2026), and `gapsByCategory`, what is missing per Kategori value (each a search; a kr column counts only against the categories it belongs to, and the photo gap only once some thing carries one). Pure and tested |
